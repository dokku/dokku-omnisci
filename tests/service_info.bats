...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "http://l:$password@dokku-omnisci-l:5984/l"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "http://test_with_underscores:$password@dokku-omnisci-test-with-underscores:5984/test_with_underscores"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  local password="$(sudo cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  assert_output "http://l:$password@dokku-omnisci-l:5984/l"
